...
class RetrieveBissControllerHipISpec extends IntegrationBaseSpec with RetrieveBissFixture {
  ...
  "Calling the retrieve BISS endpoint" should {
    ...
    "a downstream service error" when {
      ...
      downstreamErrors.foreach(args => serviceErrorTest.tupled(args))
    }
  }
  ...
  trait TysTest extends Test {
    def downstreamUrl: String = s"/itsa/income-tax/v1/income-sources/23-24/$nino/$businessId/$incomeSourceType/biss"
  }
