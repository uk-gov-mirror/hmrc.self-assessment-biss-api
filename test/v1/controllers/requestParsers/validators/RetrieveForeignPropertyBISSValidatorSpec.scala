  ...
  "running the validator" should {
    ...
    "return one error" when {
      "an invalid nino is provided" in {
        validator.validate(RetrieveForeignPropertyBISSRawData(invalidNino, Some(businessId), Some(validTypeOfBusiness), Some(taxYear))) shouldBe List(
          NinoFormatError)
      }
      "an invalid tax year is provided" in {
        validator.validate(RetrieveForeignPropertyBISSRawData(nino, Some(businessId), Some(validTypeOfBusiness), Some(invalidTaxYear))) shouldBe List(
          RuleTaxYearRangeInvalidError)
      }
      "an invalid type of business is provided" in {
        validator.validate(RetrieveForeignPropertyBISSRawData(nino, Some(businessId), Some(invalidTypeOfBusiness), Some(taxYear))) shouldBe List(
          TypeOfBusinessFormatError)
      }
      "no type of business is provided" in {
        validator.validate(RetrieveForeignPropertyBISSRawData(nino, Some(businessId), noTypeOfBusiness, Some(taxYear))) shouldBe List(
          RuleTypeOfBusinessError)
      }
      "an invalid business id is provided" in {
        validator.validate(RetrieveForeignPropertyBISSRawData(nino, Some(invalidBusinessId), Some(validTypeOfBusiness), Some(taxYear))) shouldBe List(
          BusinessIdFormatError)
      }
    ...
    "return multiplpe errors" when {
      "multiple invalid parameters are provided" in {
        ...
        validator.validate(
          RetrieveForeignPropertyBISSRawData(invalidNino, Some(businessId), Some(invalidTypeOfBusiness), Some(taxYear))) shouldBe expectedResult
      }
    }
